...
// 鋒法：行筆之寬與起收之勢
export interface 鋒法 { 寬: number; 起?: '藏' | '尖'; 收?: '回' | '出' }

// 鋒路：一筆之內記其軌（格網座標，同 Y/u 之制）
export interface 鋒路 {
...
function 執筆之(ctx: CanvasRenderingContext2D, R: number, 頂y: number): 筆具 {
  ...
  // ── 筆鋒：路徑採樣→法線偏移→寬度包絡→填充輪廓（造像標準 9.1）──────────
  type 點 = { x: number; y: number };
  const 一筆 = (法: 鋒法, 書: (p: 鋒路) => void) => {
    // 寬必有度：非有窮正數者，正之（誦戒之意，寬勿破圖）
    const 寬 = Number.isFinite(法.寬) && 法.寬 > 0 ? 法.寬 : W_IN;
    const 諸路: 點[][] = [];
    let 路: 點[] | null = null;
    let 游: 點 = { x: 0, y: 0 };
    const px = (x: number, z: number): 點 => ({ x: x * u, y: Y(z) });
    const 起 = (b: 點) => {          // 未 M 而書者，首筆自為 M 至其終點（canvas 之慣）
      if (路) return false;
      游 = b; 路 = [游]; 諸路.push(路); return true;
    };
    const 採 = (n: number, f: (t: number) => 點) => {
      for (let i = 1; i <= n; i++) 路!.push(f(i / n));
    };
    ...
    const p: 鋒路 = {
      ...
      L(x, z) {
        const b = px(x, z), a = 游;
        if (起(b)) return p;
        採(Math.min(96, Math.max(2, Math.ceil(Math.hypot(b.x - a.x, b.y - a.y) / 步))),
    ...
    for (const 原 of 諸路) {
      ...
      for (const pt of 原) {
        ...
        if (!末 || Math.hypot(pt.x - 末.x, pt.y - 末.y) > 0.05) q.push(pt);
      }
      ...
      const 寬於 = (d: number) => {
        let w = 寬;
        if (法.起 === '尖' && d < s起) w *= 0.10 + 0.90 * ease(d / s起);
        if (法.收 === '出' && d > L - s收) w *= 0.06 + 0.94 * ease((L - d) / s收);
        return w / 2;
      };
      ...
      for (let i = 0; i < q.length; i++) {
        ...
        if (n > 0.01) { nx = -dy / n; ny = dx / n; 前nx = nx; 前ny = ny; }
        else { nx = 前nx; ny = 前ny; }
        const r = 寬於(s[i]);
      ...
      if (法.起 !== '尖') ctx.arc(q[0].x, q[0].y, 寬於(0), θ0 - Math.PI / 2, θ0 + Math.PI / 2, true);
      else ctx.lineTo(左[0].x, 左[0].y);
      for (let i = 1; i <= N; i++) ctx.lineTo(左[i].x, 左[i].y);
      if (法.收 !== '出') ctx.arc(q[N].x, q[N].y, 寬於(L), θN + Math.PI / 2, θN - Math.PI / 2, true);
      else ctx.lineTo(右[N].x, 右[N].y);
      for (let i = N - 1; i >= 0; i--) ctx.lineTo(右[i].x, 右[i].y);
      ctx.closePath();
      ctx.fill();
    }
  };
...
export interface 運筆具 {
  ...
  細(fn: () => void): void;
  逸(fn: () => void): void;
  陰(fn: () => void): void;   // 陰邊重：於現階之上加寬（光自畫面左上，右下緣為陰）
  陽(fn: () => void): void;   // 陽邊輕：於現階之上減寬——陰陽線寬，立體之第一驅動
}
